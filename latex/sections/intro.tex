...
\begin{enumerate}
  \item \textbf{Regime-aware state representation.}
        ...
        return statistics with three macro-financial inputs: the SELIC target
        rate, the average bank lending spread to households, and a one-hot
        encoding of the SELIC regime as rising, stable, or falling.
        The encoding lets the agent condition portfolio decisions on the
        prevailing monetary-policy stance rather than on price history alone.
  ...
  \item \textbf{Rigorous out-of-sample evaluation.}
        The classical benchmarks are evaluated under a rolling walk-forward
        protocol with 35 quarterly folds, re-estimating weights on 252-day
        windows and measuring on the following 63 days; the offline agent is
        trained once on 2016--2021 and deployed frozen over a 2022--2024 holdout.
        Neither protocol admits look-ahead bias, and Section~\ref{sec:methodology}
        is explicit about the asymmetry between them.
